...
if (!process.env.DB_KEY) {
  console.log(chalk.yellow('Required DB_KEY enviroment variable not found.'))
  if (insideNetlify) {
    console.log(`Visit https://app.netlify.com/sites/YOUR_SITE_HERE/settings/deploys`)
    console.log('and set a `DB_KEY` value in the "Build environment variables" section')
    process.exit(1)
  }
  ...
  if (!insideNetlify) {
    ...
    ask(chalk.bold('Enter your faunaDB server key'), (err, answer) => {
      ...
      createFaunaDB(process.env.DB_KEY).then(() => {
...
function createFaunaDB(key) {
  ...
  return client.query(q.Create(q.Ref("classes"), { name: "carts" }))
    .then(()=>{
      return client.query(
        q.Create(q.Ref("indexes"), {
          name: "all_carts",
          source: q.Ref("classes/carts")
        }))
    }).catch((e) => {
